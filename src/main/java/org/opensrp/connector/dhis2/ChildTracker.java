...
public class ChildTracker extends DHIS2Service implements DHIS2Tracker {
	...
	public JSONArray getTrackCaptureData(Client client) throws JSONException {
		...
		generateTrackCaptureData.put(
		    dhis2TrackerService.withKnownValue(DHIS2Settings.CHILDMAPPING.get(firstName).toString(), client.fullName()));
		
		//Gender
		generateTrackCaptureData.put(
		    dhis2TrackerService.withKnownValue(DHIS2Settings.CHILDMAPPING.get(gender).toString(), client.getGender()));
		
		generateTrackCaptureData.put(dhis2TrackerService
		        .withKnownValue(DHIS2Settings.CHILDMAPPING.get("base_entity_id").toString(), client.getBaseEntityId()));
		
		//Child_Birth_Certificate
		generateTrackCaptureData.put(dhis2TrackerService.getTrackCaptureData(attributesAsJson,
		    DHIS2Settings.CHILDMAPPING.get(Child_Birth_Certificate).toString(), Child_Birth_Certificate));
		
		//birthdate	
		generateTrackCaptureData.put(dhis2TrackerService
		        .withKnownValue(DHIS2Settings.CHILDMAPPING.get("birthdate").toString(), client.getBirthdate().toString()));
		...
		generateTrackCaptureData
		        .put(dhis2TrackerService.withKnownValue(DHIS2Settings.COMMONMAPPING.get("client_type").toString(), "Child"));
