...
public class OpenmrsReportingService extends OpenmrsService {
    ...
    public JSONArray getReportDefinitions() throws JSONException {
        JSONArray res = new JSONObject(
                HttpUtil.get(getURL() + "/" + REPORT_DEFINITION_URL, "v=full", OPENMRS_USER, OPENMRS_PWD).body())
    ...
    public JSONArray getReportData(String uuid, Map<String, String> params) throws JSONException {
        ...
        JSONArray ds = new JSONObject(
                HttpUtil.get(getURL() + "/" + REPORT_DATA_URL + "/" + uuid, payload, OPENMRS_USER, OPENMRS_PWD).body())
