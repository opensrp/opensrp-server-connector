...
public class EncounterAtomfeed extends OpenmrsService implements EventWorker, AtomfeedService {
	...
	public EncounterAtomfeed(AllMarkers allMarkers, AllFailedEvents allFailedEvents,
	    @Value("#{opensrp['openmrs.url']}") String baseUrl, EncounterService encounterService, EventService eventService)
	...
	public void process(Event event) {
		...
		try {
			...
			JSONObject encounter = encounterService.getEncounterByUuid(content, true);
			if (encounter == null) {
				throw new RuntimeException(
				        "Encounter uuid specified in atomfeed content (" + content + ") did not return any encounter.");
			}
			
			log.info(encounter.toString());
			
			org.opensrp.domain.Event enc = encounterService.convertToEvent(encounter);
			org.opensrp.domain.Event existing = eventService.find(encounter.getString("uuid"));
			if (existing == null) {
				log.info("New Event");
