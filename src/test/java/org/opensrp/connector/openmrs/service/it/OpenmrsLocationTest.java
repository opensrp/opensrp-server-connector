...
public class OpenmrsLocationTest extends OpenmrsApiService {
	...
	@Test
	public void addUpdateDeleteSearchLocation() {
		try {
			JSONObject response = new JSONObject();
			response = ls.addLocation(makeLocationsObject(LOCATIONNAE).toString());
			String actualLocationName = response.getString(nameKey);
			assertEquals(LOCATIONNAE, actualLocationName);
			String uuid = response.getString(uuidKey);
			JSONObject updatedResponse = new JSONObject();
			updatedResponse = ls.updateLocation(makeLocationsObject(LOCATIONNAMEUPDATED).toString(), uuid);
			String actualUpdatedLocationName = updatedResponse.getString(nameKey);
			assertEquals(LOCATIONNAMEUPDATED, actualUpdatedLocationName);
			searchLocations(LOCATIONNAMEUPDATED);
			int actualStatusCode = ls.deleteLocation(uuid);
			assertEquals(204, actualStatusCode);
	...
	private void searchLocations(String name) {
		JSONObject searchResponse = new JSONObject();
		searchResponse = ls.searchLocations(name, 0, 0);
	...
	private JSONObject makeLocationsObject(String name) throws JSONException {
